...
<div class="container-fluid px-4">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2 md:mb-0">Edit Video</h1>
    </div>
    ...
    <nav class="mb-6" aria-label="Breadcrumb">
        <ol class="flex text-sm text-gray-500 overflow-x-auto whitespace-nowrap">
            ...
            <li class="flex items-center">
                ...
                <span class="text-gray-700 font-medium" aria-current="page">Edit Video</span>
            </li>
        </ol>
    </nav>
        ...
        <div class="p-5">
            ...
            <form action="{{ route('admin.videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="title" name="title" value="{{ old('title', $video->title) }}" required>
                        </div>
                        
                        <div>
                            <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="description" name="description" rows="5">{{ old('description', $video->description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="category" name="category">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category }}" {{ old('category', $video->category) == $category ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="duration" name="duration" value="{{ old('duration', $video->duration) }}">
                        </div>
                    </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="video-url-group">
                            ...
                            <input type="text" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="video_url" name="video_url" value="{{ old('video_url', $video->video_url) }}" 
                        ...
                        <div class="mb-3 video-file-group">
                            ...
                            @if($video->video_file)
                                <div class="mb-2 flex items-center text-green-600">
                                    ...
                                    <span>Current file: {{ basename($video->video_file) }}</span>
                                </div>
                            @endif
                            <input type="file" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" id="video_file" name="video_file" accept="video/mp4,video/mov,video/avi,video/wmv">
                        ...
                        <div>
                            ...
                            @if($video->thumbnail)
                                <div class="mb-3">
                                    <img src="{{ $video->thumbnail_url }}" alt="Current thumbnail" class="h-24 w-auto object-cover rounded-md shadow-sm" loading="lazy">
                                </div>
                            @endif
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="flex items-center">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer" id="featured" name="featured" value="1" {{ old('featured', $video->featured) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="flex items-center">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer" id="published" name="published" value="1" {{ old('published', $video->published) ? 'checked' : '' }}>
                                    <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        ...
                        <div>
                            ...
                            <input type="date" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="publish_date" name="publish_date" value="{{ old('publish_date', $video->publish_date ? $video->publish_date->format('Y-m-d') : '') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end mt-8 space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Update Video</button>
                </div>
            </form>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        $('#video_type').change(function() {
            ...
            if (selectedType === 'youtube' || selectedType === 'vimeo') {
                ...
                $('#video_file').attr('required', false); // Not required on edit as file may already exist
            } else {
                $('.video-url-group').addClass('hidden');
                $('.video-file-group').addClass('hidden');
                $('#video_url').attr('required', false);
